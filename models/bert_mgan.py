...
class Bert_MGAN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        #torch.tensor(aspect_in_text)
        ctx, asp, ctx_len, asp_len = self.bert_embedding(inputs)
        ...
        alignment_mat = self.alignment(self.opt.batch_size, ctx_out, asp_out) # batch_size x (ctx)seq_len x (asp)seq_len
        ...
        c_asp2ctx_alpha = F.softmax(ctx_out.matmul(self.w_a2c.expand(self.opt.batch_size, -1, -1)).matmul(asp_pool), dim=1)
        c_asp2ctx = torch.matmul(ctx_out.transpose(1, 2), c_asp2ctx_alpha).squeeze(-1)
        c_ctx2asp_alpha = F.softmax(asp_out.matmul(self.w_c2a.expand(self.opt.batch_size, -1, -1)).matmul(ctx_pool), dim=1)
