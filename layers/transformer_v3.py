...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        for layer in self.layers:
            x = layer(x, mask)
        x = self.norm(x)
        # return representation of token [cls]
        return x
...
class Embeddings(nn.Module):
    def __init__(self, we, vocab, embed_dim, d_model):
        super(Embeddings, self).__init__()
        word_embedding = torch.from_numpy(we).float()
        self.embed = nn.Embedding(vocab, embed_dim, _weight=word_embedding)
        self.lut = nn.Linear(embed_dim, d_model)
...
class Transformer(nn.Module):
    ...
    def __init__(self, vocab_size, word_embedding):
        super(Transformer, self).__init__()
        embed_dim = len(word_embedding[0])
        d_model = len(word_embedding[0])
        h = 4
        d_ff = 1024
        N = 6
        dropout = 0.1
